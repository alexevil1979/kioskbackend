...
### Автоматически с GitHub (рекомендуется)

1. Закройте киоск (окно приложения или завершите `python` / `pythonw` в диспетчере задач).
2. Запустите **`C:\kiosk\update_kiosk.bat`** (нужен интернет).
3. Скрипт скачает ZIP ветки `main` с https://github.com/alexevil1979/kioskbackend и развернёт в `C:\kiosk`.
4. Не перезаписываются: `config/settings.yaml`, `.env`, `logs/`, `media/products/`.
5. После копирования выполняется `install.ps1` (обновление pip-зависимостей).
6. Запустите `run_kiosk.bat` и проверьте экраны.

### Вручную (ZIP с флешки)
